...
freq_values = np.arange(100.0, 601.0, 1.0, dtype=np.float64)
frf_matrix  = np.zeros((np.shape(rho_values)[0],np.shape(freq_values)[0]))
...
for cont1 in range(n_rho):
    ...
    frf_matrix[cont1,:] = np.loadtxt(filn, skiprows=3)[99:600,1].T
# end for
...
def prepare_data(
    ...
    frf_matrix:  np.ndarray,   # (n_rho, n_f)  real-valued H(ρ, f) from Abaqus
...
def train_full_model(
    ...
    X_train, y_mag, y_sign, meta = prepare_data(rho_values, frf_matrix, freq_values)
...
fig_hist.savefig("CUDA_training_history.png", dpi=150, bbox_inches="tight")
plt.close(fig_hist)
